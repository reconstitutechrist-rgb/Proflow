...
export default function ContextualContentHub({ currentUser, assignmentId }) {
  // Changed prop from selectedAssignment to assignmentId
    ...
    try {
      ...
      const [
        ...
        Message.filter(
          ...
          '-created_date',
          20
        ).catch(() => []), // Added catch to prevent allSettled from failing if messages fail
        Assignment.filter(
          {
            workspace_id: currentWorkspaceId,
          },
      ...
      setRawContextualData({
        assignment:
          currentAssignmentPromise.status === 'fulfilled' ? currentAssignmentPromise.value : null, // Pass the full assignment object to AI
        ...
        relatedAssignments: (relatedAssignmentsPromise.status === 'fulfilled'
          ? relatedAssignmentsPromise.value
          : []
        )
      ...
      setCurrentAssignmentDetails(
        currentAssignmentPromise.status === 'fulfilled' ? currentAssignmentPromise.value : null
      );
    } catch (error) {
  ...
  const generateContentInsights = useCallback(
...
Please keep it friendly and practical - like advice from a helpful colleague who's been watching how the team works and wants to share some useful observations.`;
    ...
    [currentUser, isCacheValid, contentInsights, rawContextualData, isDataLoading]
  ); // Dependencies for generateContentInsights
  ...
  if (isDataLoading || isGeneratingInsights) {
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="connections" className="mt-4">
            <div className="space-y-4">
              {contentInsights.content_connections?.length > 0 ? (
                contentInsights.content_connections.map((connection, index) => (
                  <div key={index} className="p-4 border border-gray-200 rounded-lg">
                    <div className="flex items-start justify-between mb-2">
                      <h4 className="font-medium text-gray-900">{connection.title}</h4>
                      <Badge
                        className={`text-xs ${getConnectionStrengthColor(connection.strength)}`}
                      >
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm mt-1">
                    As you add more content, I'll start seeing patterns!
                  </p>
                </div>
              )}
            </div>
          ...
          <TabsContent value="patterns" className="mt-4">
            <div className="space-y-4">
              {contentInsights.workflow_patterns?.length > 0 ? (
                contentInsights.workflow_patterns.map((pattern, index) => (
                  <div key={index} className="p-4 border border-gray-200 rounded-lg">
                    <div className="flex items-start justify-between mb-2">
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm mt-1">
                    As your team works, I'll learn more about your processes!
                  </p>
                </div>
              )}
            </div>
          ...
          <TabsContent value="knowledge" className="mt-4">
            <div className="space-y-4">
              {contentInsights.knowledge_opportunities?.length > 0 ? (
                contentInsights.knowledge_opportunities.map((opportunity, index) => (
                  <div key={index} className="p-4 border border-gray-200 rounded-lg">
                    <div className="flex items-start justify-between mb-2">
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm mt-1">
                    Keep documenting and sharing, and I'll highlight areas for improvement!
                  </p>
                </div>
              )}
            </div>
          ...
          <TabsContent value="collaboration" className="mt-4">
            <div className="space-y-4">
              {contentInsights.collaboration_insights?.length > 0 ? (
                contentInsights.collaboration_insights.map((insight, index) => (
                  <div key={index} className="p-4 border border-gray-200 rounded-lg">
                    <div className="flex items-start justify-between mb-2">
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm mt-1">
                    As your team interacts, I'll observe and share tips!
                  </p>
                </div>
              )}
            </div>
