...
export default function AudienceRewriter({ document, onRewriteComplete }) {
  ...
  const audienceOptions = useMemo(() => [ // Use useMemo for constant data
    { value: "general", label: "General Audience", description: "Clear and accessible to everyone" },
    { value: "technical", label: "Technical Experts", description: "Detailed and technical language" },
      ...
      >
        {rewriting ? (
          <>
            ...
            Rewriting for {audienceOptions.find(opt => opt.value === targetAudience)?.label || "audience"}...
          </>
        ) : (
          <>
            <Sparkles className="w-4 h-4 mr-2" />
            Rewrite for {audienceOptions.find(opt => opt.value === targetAudience)?.label || "Audience"}
          </>
        )}
      </Button>
