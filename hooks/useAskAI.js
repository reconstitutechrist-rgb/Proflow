...
export function useAskAI() {
  ...
  const saveDraftToStorage = useCallback(() => {
    try {
      const { messages, uploadedDocuments, selectedAssignment, selectedProject, contextType, totalEmbeddingCost } = autoSaveDataRef.current;
      const draft = {
        ...
        try {
          ...
          const extractResult = await ExtractDataFromUploadedFile({
            ...
            json_schema: {
              type: "object",
              properties: {
                full_text: { type: "string", description: "The complete text content extracted from the PDF" },
  ...
  const handleSendMessage = async () => {
    ...
    try {
        ...
        try {
          ...
          if (allChunks.length > 0) {
            ...
            if (data && Array.isArray(data.chunks)) {
              ...
              ragMetadata.chunkTypes = [...new Set(relevantChunks.map(c => c.chunkType || 'unknown'))];
      ...
      if (selectedProject) {
        ...
        if (selectedProject.description) systemPrompt += `\nDescription: ${selectedProject.description}`;
      }

      if (selectedAssignment) {
        systemPrompt += `\n\nCurrent Assignment: ${selectedAssignment.name}`;
        if (selectedAssignment.description) systemPrompt += `\nDescription: ${selectedAssignment.description}`;
      }
      ...
      if (contextData.length > 0) {
        ...
        contextData.forEach((ctx, idx) => {
          ...
          if (ctx.relevanceScore) systemPrompt += ` (Relevance: ${(ctx.relevanceScore * 100).toFixed(1)}%)`;
    ...
    try {
      ...
      if (format === "pdf") {
        ...
        if (selectedProject) {
          ...
          if (selectedProject.description) markdown += `Description: ${selectedProject.description}\n`;
          markdown += `\n`;
        }

        if (selectedAssignment) {
          markdown += `**Assignment:** ${selectedAssignment.name}\n`;
          if (selectedAssignment.description) markdown += `Description: ${selectedAssignment.description}\n`;
          markdown += `\n`;
        }
