    ...
    return (
        <>
            <motion.div
                ...
                exit={{ opacity: 0, y: -20 }}
            >
                <Card className={`bg-white/80 backdrop-blur-sm hover:shadow-lg transition-all duration-300 ${isOverdue ? 'border-l-4 border-l-red-500' : ''}`}>
                    <CardHeader className="flex flex-row items-start justify-between pb-4">
                        <div className="flex items-start gap-3 flex-1">
                            ...
                            <div className="flex-1 min-w-0">
                                <CardTitle className={`text-lg ${task.status === 'completed' ? 'line-through text-gray-500' : 'text-gray-900'}`}>
                                    {task.title}
                                    {task.auto_generated && (
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        <Avatar className="w-5 h-5">
                                            <AvatarFallback className="text-xs">
                                                {assignedUser.full_name?.split(' ').map(n => n[0]).join('') || 'U'}
                                            </AvatarFallback>
                                        </Avatar>
                                        <span className="text-sm text-gray-600">{assignedUser.full_name || assignedUser.email}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <CardContent className="pt-0">
                        <div className="flex items-center gap-2 flex-wrap mb-4">
                            ...
                            {task.due_date && (
                                <Badge variant="outline" className={`flex items-center gap-1 ${isOverdue ? 'bg-red-50 text-red-700 border-red-200' : ''}`}>
                        ...
                        {task.auto_generated && task.generation_source && (
                            <div className="mb-4 p-2 bg-purple-50 border border-purple-200 rounded text-xs">
                                <strong>Generated from:</strong> {task.generation_source.source_type.replace('_', ' ')}
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="space-y-3">
                            ...
                            <p className="text-sm text-gray-600">This action cannot be undone and will permanently remove:</p>
                            ...
                            {task.auto_generated && (
                                <div className="p-3 bg-amber-50 border border-amber-200 rounded text-sm">
                                    <p className="text-amber-800">
                                        <strong>Note:</strong> This is an AI-generated task. Deleting it won't affect the source document or workflow pattern.
                                    </p>
                                </div>
                            )}
                            {deleteError && (
