  ...
  const loadWorkflowPatterns = async () => {
    try {
      ...
      const workflowPatterns = await db.entities.WorkflowPattern.filter(
        ...
        '-usage_count',
        10
      ); // Filter by usage count descending, limit 10
...
Return the pattern name that best matches, or suggest a custom pattern.`;
  ...
  const handleApplyPattern = async (pattern) => {
    ...
    if (
      assignment.workspace_id !== currentWorkspaceId ||
      pattern.workspace_id !== currentWorkspaceId
    ) {
    ...
    try {
      ...
      for (const stage of pattern.pattern_sequence || []) {
        // Ensure pattern_sequence is iterable
        for (const taskTitle of stage.typical_tasks || []) {
          // Ensure typical_tasks is iterable
          tasksToCreate.push({
      ...
      toast.success(
        `Applied '${pattern.name}' workflow pattern - ${tasksToCreate.length} tasks created`
      );
          ...
          <div className="border border-purple-200 bg-purple-50 rounded-lg p-4">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <Badge className="bg-purple-200 text-purple-800 text-sm">
                Confidence:{' '}
                {suggestedPattern.ai_confidence || suggestedPattern.success_rate || 'N/A'}%
              </Badge>
            </div>
            <p className="text-sm text-purple-700 mb-3">
              {suggestedPattern.ai_reasoning || suggestedPattern.description}
            </p>
            {suggestedPattern.isCustomSuggestion ? (
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() => {
                    ...
                    toast.info(
                      'This is a new AI-suggested pattern. Define its stages to save and apply.'
                    );
              ...
              >
                {applying ? (
                  ...
                  <>
                    <BookOpen className="w-4 h-4 mr-1" /> Apply Suggested Pattern
                  </>
                )}
              </Button>
            )}
        ...
        <div className="space-y-4">
          ...
          {patterns.length === 0 && !loading && (
            <p className="text-gray-500 text-sm">
              No workflow patterns found for this workspace. Create a new one?
            </p>
          )}
