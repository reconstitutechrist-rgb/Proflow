...
export default function AIWritingAssistant({
  ...
  const modes = [
    ...
    },
    {
      ...
      systemPrompt: "Improve clarity, grammar, and tone. Make the text more professional and engaging.",
    ...
    <div className="space-y-6">
          ...
          {modes.map(m => (
            ...
            >
              <div className="flex items-center gap-3">
                <m.icon className={`w-5 h-5 ${mode === m.value ? 'text-gray-900 dark:text-white' : 'text-gray-400'}`} />
                <div className="flex-1">
                  <div className={`font-medium text-sm ${mode === m.value ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300'}`}>
                    {m.label}
                  </div>
      ...
      {mode === "refine" && (
        <Alert className="bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800">
          ...
          <AlertDescription className="text-xs text-blue-900 dark:text-blue-100">
            <strong>Tip:</strong> Select text in the editor to improve that specific section, or leave nothing selected to improve the beginning of your document.
          </AlertDescription>
        </Alert>
      )}
      ...
      {!generatedText && !isGenerating && (
        <div className="text-center py-12 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-lg">
          ...
          <p className="text-sm font-medium text-gray-600 dark:text-gray-400">{currentMode.label}</p>
