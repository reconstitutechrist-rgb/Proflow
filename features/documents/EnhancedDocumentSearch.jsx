...
import { Search, Brain, FileText, Eye, Filter, Sparkles, Loader2 } from 'lucide-react';
  ...
  const loadDocuments = useCallback(async () => {
    ...
    try {
      ...
      const docs = await Document.filter(
        // Using imported Document entity
...
Return 5-8 related terms and synonyms as JSON array.`;
      ...
      const allTerms = [
        query.toLowerCase(),
        ...(response.keywords || []).map((k) => k.toLowerCase()),
      ];
  ...
  const performDocumentSearch = useCallback(
    ...
    [documents, selectedAssignment, documentType, expandDocumentQuery]
  ); // Dependencies: documents, filters, and expandDocumentQuery
  ...
  React.useEffect(() => {
    ...
    documents.length,
    loading,
  ]); // Added documents.length and loading as dependencies
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      {searchMode === 'semantic' &&
        expandedQuery &&
        expandedQuery.toLowerCase() !== searchQuery.toLowerCase() && (
