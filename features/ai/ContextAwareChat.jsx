      ...
      <div key={msg.id || index} className={`flex gap-3 mb-6 ${isOwnMessage ? 'justify-end' : ''}`}>
        ...
        <div
          className={`max-w-lg rounded-lg ${
            isOwnMessage ? 'bg-blue-600 text-white' : 'bg-white shadow-sm border'
          }`}
        >
          {/* Message Header */}
          ...
          <div className="px-4 pb-3">
            {msg.message_type === 'file' ? (
              <div>
                ...
                >
                  <FileText
                    className={`w-5 h-5 ${isOwnMessage ? 'text-blue-200' : 'text-gray-500'}`}
                  />
                ...
                {msg.decision_details && (
                  <div className={`p-3 rounded-lg ${isOwnMessage ? 'bg-blue-700' : 'bg-green-50'}`}>
                    <div className="space-y-2">
                      <p className="font-medium text-sm">{msg.decision_details.decision_title}</p>
                      <p className="text-sm opacity-90">{msg.decision_details.decision_summary}</p>
                      ...
                      {msg.decision_details.due_date && (
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span>
                            Due: {new Date(msg.decision_details.due_date).toLocaleDateString()}
                          </span>
                        </div>
                      )}
                    </div>
              ...
              <div className="flex flex-wrap gap-1 mt-3">
                {msg.tags.map((tag) => (
                  <Badge
                    key={tag}
                    className={`text-xs ${
                      isOwnMessage ? 'bg-blue-500 text-blue-100' : 'bg-gray-100 text-gray-600'
                    }`}
                  >
                    <Hash className="w-3 h-3 mr-1" />
                    {tag}
                    ...
                    <div className="space-y-2">
                      {contextSuggestions.suggested_documents.map((docId) => {
                        ...
                        return doc ? (
                          <div
                            key={docId}
                            className="flex items-center gap-2 p-2 rounded-md bg-white border border-gray-100 shadow-sm"
                          >
