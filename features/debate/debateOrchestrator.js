...
const SYSTEM_PROMPTS = {
  analyst: `You are an expert code analyst working as part of a dual-AI system. Your role is to provide thorough, well-reasoned analysis based on the user's questions and the provided context.

CRITICAL: You may have access to VERIFIED knowledge from the context. This is real, verified information. USE IT as your primary source of truth.

Your approach:
1. Analyze the provided context first (if any)
2. Reference specific details from the verified context
3. Provide clear, structured answers with specific examples
...
- Specific references to context (when available)
- Confidence levels for your conclusions
- Key points that you believe are well-supported`,

  critic: `You are an expert reviewer working as part of a dual-AI system. Your role is to critically evaluate the Analyst's responses and ensure accuracy against the provided context.

CRITICAL: You may have access to VERIFIED knowledge from the context. Use this to verify the Analyst's claims. Call out any hallucinations or unsupported claims.

Your approach:
1. Verify the Analyst's claims against the provided context
...
- Flag any claims that aren't supported by the context
...
export const runDebateRound = async (context, role, invokeAI) => {
  ...
  if (role === 'analyst' && context.currentRound === 1) {
    ...
    userMessage = `Please analyze this based on the user's question. Use the context provided above.`;
...
async function persistDebateInsights(context, sessionInfo) {
  ...
  // Save to database with embeddings (repositoryId is now optional)
